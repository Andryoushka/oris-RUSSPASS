namespace RUSSPASS.API.Data;

public static class DataSeed
{
    public static object[] GetTours()
    {
        return new object[]
        {
            new
            {
                name = "Тур по столовке КФУ от Тимерхана Аглямыча",
                card_image = "https://sun9-36.userapi.com/s/v1/ig1/KEjh6RQDfICyR9Ah3EovNvFKciBRx-Zr2qigaQSCC-3_36nn_A9YWHTqv5B1836jETQy_wbn.jpg?quality=96&as=32x32,48x48,72x72,108x108,160x160,240x240,360x360,480x480,540x540,640x640,720x720,1080x1080,1280x1280,1440x1440,2160x2160&from=bu&cs=2160x0",
                location = "Татмач",
                type = "tour",
                hotelStars = 0,
                duration = 1,
                features = new int[] { 0, 2, 7 },
                price = 100,
                gallery = new string[]
                {
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0",
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0",
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0"
                },
                description = "Гастрономическое приключение для бедных студентов. Порадуй свой желудок чем-то кроме доширака!",
                tour_program = new
                {
                    days = new object[]
                    {
                        new
                        {
                            events = new string[]
                            {
                                "Встреча студентов в столовой в 08:00",
                                "Не забудь захватить студенческий!",
                                "Вкусно кушаем, никого не слушаем"
                            }
                        }
                    }
                }
            },
            new
            {
                name = "Тур по Основам разработки современных приложений от Тимерхана Аглямыча",
                card_image = "https://sun9-30.userapi.com/s/v1/ig1/39gOnJHS57KR3h1rZaXNbtUZGNNVl1ZNazWfCB2oQqj_7KdxMw3GvDLfdJIbxuFpkq_s5TIV.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,1080x1440,1280x1707,1440x1920,1620x2160&from=bu&cs=1620x0",
                location = "КФУ ИТИС",
                type = "tour",
                hotelStars = 2,
                duration = 2,
                features = new int[] { 1 },
                price = 200,
                gallery = new string[]
                {
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0",
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0",
                    "https://sun9-32.userapi.com/s/v1/ig2/hAyvhnQLp0V4WuRLTZvV4hdZ3p3xLGFvKiCMCEL9XX9qBGGD-4qzLyfpJqj50ZiGxhiA5wfHmEydrx87VNXKVLlw.jpg?quality=96&as=32x43,48x64,72x96,108x144,160x213,240x320,360x480,480x640,540x720,640x853,720x960,768x1024&from=bu&cs=768x0"
                },
                description = "Ведущий специалист по разработке программного обеспечения расскажет и покажет КАК ЭТО ДЕЛАЕТСЯ!",
                tour_program = new
                {
                    days = new object[]
                    {
                        new
                        {
                            events = new string[]
                            {
                                "Ввведение в дисциплину",
                                "Знакомство с группой"
                            }
                        },
                        new
                        {
                            events = new string[]
                            {
                                "Знакомство с ASP.NET",
                                "Изучаем React"
                            }
                        }
                    }
                }
            }
        };
    }
}